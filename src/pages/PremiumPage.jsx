...
export default function PremiumPage() {
  ...
  const ADMIN_WHATSAPP = "2348000000000"; // ⚠️ Replace with your WhatsApp number (include country code without +)
  ...
  async function handleProcessPayment(e) {
    ...
    try {
      // 1. Generate path details natively
      const fileExt = file.name.split(".").pop();
      const fileName = `${user?.id ?? "anon"}-${Date.now()}.${fileExt}`;
      const filePath = `receipts/${fileName}`;

      // 2. Transmit binary payload to Storage bucket
      ...
      // 3. Resolve downstream public URL access
      const { data: { publicUrl } } = supabase.storage
        .from("payment_receipts")
        .getPublicUrl(filePath);

      setStatus("Receipt verified! Forwarding to validation...");

      // 4. Construct deep link payload string for WhatsApp validation
      const baseMessage = `Hello Admin! I just paid ₦2,000 for Gradia Premium.\n\n` +
                          `🔑 User ID: ${user?.id || "N/A"}\n` +
                          `📧 Email: ${user?.email || "N/A"}\n` +
                          `🖼️ Receipt: ${publicUrl}\n\n` +
                          `Please confirm and activate my premium access.`;

      const whatsappUrl = `https://wa.me/${ADMIN_WHATSAPP}?text=${encodeURIComponent(baseMessage)}`;
      ...
      setStatus("Failed to process receipt. Please verify database storage settings.");
    } finally {
      setUploading(false);
    }
  }
  ...
  return (
    <div className="min-h-screen bg-[#f5f9f2] dark:bg-[#0b160a] font-sans pb-16">
      ...
      <main className="max-w-md mx-auto px-4 mt-6 space-y-6">
        ...
        {/* Upload Interface Section Container */}
        <section className="space-y-3">
          ...
          {status && (
            <div className={`p-3 rounded-xl border text-center text-[11px] font-bold tracking-tight animate-pulse ${
